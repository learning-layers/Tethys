...
//	curl -i http://137.226.58.142:35357/v2.0/users -X GET  -H "X-Auth-Token: "
	...
	public static JsonObject verifyAccessToken(String accessToken) {
		...
		ClientResponse response = tokens.request().header("Authorization", "Bearer " + accessToken).get(ClientResponse.class);

		JsonObject output = new JsonObject();
		if(response.getStatusInfo()==Status.OK) {
			output = response.readEntity(JsonObject.class);
		}else {
			output.addProperty("StatusCode", response.getStatusInfo().getStatusCode());
		}
